    ...
    <section className="bigcta" id="empezar">
      ...
      <div className="container-wrap bigcta-inner">
        ...
        <div className="cta-chips">
          {chips.map((chip) => (
            <CTA key={chip} href="/contacto" variant="ghost" onDark event="advisory_click" eventParams={{ from: 'final_cta_chip', chip }}>
              {chip}
            </CTA>
          ))}
        </div>
      ...
      <style>{`
        ...
        .bigcta .cta-chips { margin-top: 22px; display: flex; flex-wrap: wrap; justify-content: center; gap: 10px; }
        .bigcta .cta-chips :global(.btn) { padding: 8px 16px; font-size: 13px; }
      `}</style>
    </section>
  );
}
