...
export function ContactForm({ source = 'contacto' }: { source?: string }) {
  ...
  const manageOptions = t.raw('manageOptions') as string[];
  ...
  return (
    <form className="lead-form" onSubmit={onSubmit} noValidate>
      ...
      <div className="field">
        ...
        <select id="manage" name="manage" defaultValue="">
          <option value="" disabled>{t('manageSelect')}</option>
          {manageOptions.map((m) => <option key={m} value={m}>{m}</option>)}
        </select>
      </div>
