...
export function WhatsAppButton({ locale }: { locale: string }) {
  ...
  return (
    ...
    >
      ...
      <style jsx>{`
        .wa-float { position: fixed; right: 20px; bottom: max(20px, env(safe-area-inset-bottom)); z-index: 90; display: flex; align-items: center; background: #25D366; color: #062b14; border-radius: 30px; box-shadow: 0 12px 32px rgba(37,211,102,.4); overflow: hidden; transition: .25s; }
        ...
        @media (max-width: 520px) { .wa-float { right: 14px; bottom: max(14px, env(safe-area-inset-bottom)); } .wa-ic { width: 50px; height: 50px; } .wa-ic svg { width: 26px; height: 26px; } }
      `}</style>
    </a>
  );
}
